...
#if (defined(newNTL_GMP_LIP) || defined(newNTL_GMP_HACK))
#define KARX 200
#else
#define KARX 80
#endif
...
void mul(ZZ_pX& x, const ZZ_pX& a, const ZZ_pX& b)
{
   ...
   if (deg(a) > newNTL_ZZ_pX_FFT_CROSSOVER && deg(b) > newNTL_ZZ_pX_FFT_CROSSOVER)
      FFTMul(x, a, b);
   else
      PlainMul(x, a, b);
}
...
{
   if (deg(a) > newNTL_ZZ_pX_FFT_CROSSOVER)
      FFTSqr(x, a);
   else
      PlainSqr(x, a);
}
...
void FFTRep::SetSize(long NewK)
{
   ...
   if (MaxK == -1) {
      tbl = (long **) newNTL_MALLOC(NumPrimes, sizeof(long *), 0);
      if (!tbl)
...
void ZZ_pXModRep::SetSize(long NewN)
{
   ...
   if (MaxN == 0) {
      tbl = (long **) newNTL_MALLOC(ZZ_pInfo->NumPrimes, sizeof(long *), 0);
      if (!tbl)
   ...
   for (i = 0; i < ZZ_pInfo->NumPrimes; i++) {
      if ( !(tbl[i] = (long *) newNTL_MALLOC(NewN, sizeof(long), 0)) )
         Error("out of space in ZZ_pXModRep::SetSize()");
   }
...
void rem21(ZZ_pX& x, const ZZ_pX& a, const ZZ_pXModulus& F)
{
   ...
   if (!F.UseFFT || da - n <= newNTL_ZZ_pX_FFT_CROSSOVER) {
      PlainRem(x, a, F.f);
      return;
   }
...
void rem(ZZ_pX& x, const ZZ_pX& a, const ZZ_pXModulus& F)
{
   ...
   else if (!F.UseFFT || da - n <= newNTL_ZZ_pX_FFT_CROSSOVER) {
      PlainRem(x, a, F.f);
      return;
   }
...
void MulMod(ZZ_pX& x, const ZZ_pX& a, const ZZ_pX& b, const ZZ_pXModulus& F)
{
   ...
   if (!F.UseFFT || da <= newNTL_ZZ_pX_FFT_CROSSOVER || db <= newNTL_ZZ_pX_FFT_CROSSOVER) {
      ZZ_pX P1;
      mul(P1, a, b);
      rem(x, P1, F);
      return;
...
void SqrMod(ZZ_pX& x, const ZZ_pX& a, const ZZ_pXModulus& F)
{
   ...
   if (!F.UseFFT || da <= newNTL_ZZ_pX_FFT_CROSSOVER) {
      ZZ_pX P1;
      sqr(P1, a);
      rem(x, P1, F);
      return;
...
void build(ZZ_pXModulus& x, const ZZ_pX& f)
{
   ...
   if (x.n <= newNTL_ZZ_pX_FFT_CROSSOVER + 1) {
      x.UseFFT = 0;
      return;
   }
                         ...
                         const ZZ_pXModulus& F)
{
   ...
   if (!F.UseFFT || db <= newNTL_ZZ_pX_FFT_CROSSOVER) {
      x.UseFFT = 0;
      return;
   }
                                      ...
                                      const ZZ_pXModulus& F)
{
   ...
   if (!B.UseFFT || !F.UseFFT || da <= newNTL_ZZ_pX_FFT_CROSSOVER) {
      ZZ_pX P1;
      mul(P1, a, B.b);
      rem(x, P1, F);
      return;
