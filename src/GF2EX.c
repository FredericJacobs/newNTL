...
void ExtractBits(_newntl_ulong *cp, const _newntl_ulong *ap, long k, long n)

// extract k bits from a at position n

{
   long sc = (k + newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG;
   ...
   if (bn == 0) {
      ...
      if (k > sc*newNTL_BITS_PER_LONG - bn) 
         cp[sc-1] = (ap[sc+wn-1] >> bn)|(ap[sc+wn] << (newNTL_BITS_PER_LONG - bn));
      else
         cp[sc-1] = ap[sc+wn-1] >> bn;
   }

   long p = k % newNTL_BITS_PER_LONG;
   if (p != 0) 
      cp[sc-1] &= ((1UL << p) - 1UL);
...
void KronSubst(GF2X& aa, const GF2EX& a)
{
   ...
   long wsaa = (saa + newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG;

   aa.xrep.SetLength(wsaa+1);

   _newntl_ulong *paa = aa.xrep.elts();
...
void KronMul(GF2EX& x, const GF2EX& a, const GF2EX& b)
{
   ...
   if (newNTL_OVERFLOW(blocksz, sx, 0))
   ...
   long wc = (blocksz + newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG;
   ...
   wc = (last_blocksz + newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG;
   c.xrep.SetLength(wc);
...
void build(GF2EXModulus& F, const GF2EX& f)
{
   ...
   if (newNTL_OVERFLOW(n, GF2E::degree(), 0))
...
newNTL_vector_impl(GF2EX,vec_GF2EX)

newNTL_eq_vector_impl(GF2EX,vec_GF2EX)

newNTL_io_vector_impl(GF2EX,vec_GF2EX)
...
void build(GF2EXArgument& A, const GF2EX& h, const GF2EXModulus& F, long m)
{
   ...
   if (GF2EXArgBound > 0) {
      ...
      sz = sz*F.n;
      sz = sz + sizeof(vec_GF2E); //todo: was + newNTL_VECTOR_HEADER_SIZE
      sz = sz/1024;
...
{
   if (k < 0 || newNTL_OVERFLOW(k, 1, 0) || deg(a) >= F.n) 
...
void reverse(GF2EX& x, const GF2EX& a, long hi)
{
   ...
   if (newNTL_OVERFLOW(hi, 1, 0)) Error("overflow in reverse");
