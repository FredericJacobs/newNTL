...
void SetCoeff(ZZ_pEX& x, long i, const ZZ_p& aa)
{
   ...
   newNTL_ZZ_pRegister(a);  // watch out for aliases!
   a = aa;
...
{
   newNTL_ZZ_pRegister(T);   // avoids aliasing problems
   T = a;
   negate(x, b);
   add(x, x, T);
}
...
void mul(ZZ_pEX& c, const ZZ_pEX& a, const ZZ_pEX& b)
{
   ...
   if (newNTL_OVERFLOW(da+db+1, n2, 0))
...
void sqr(ZZ_pEX& c, const ZZ_pEX& a)
{
   ...
   if (newNTL_OVERFLOW(2*da+1, n2, 0))
...
void build(ZZ_pEXModulus& F, const ZZ_pEX& f)
{
   ...
   if (newNTL_OVERFLOW(n, ZZ_pE::degree(), 0))
...
newNTL_vector_impl(ZZ_pEX,vec_ZZ_pEX)

newNTL_eq_vector_impl(ZZ_pEX,vec_ZZ_pEX)

newNTL_io_vector_impl(ZZ_pEX,vec_ZZ_pEX)
...
void build(ZZ_pEXArgument& A, const ZZ_pEX& h, const ZZ_pEXModulus& F, long m)
{
   ...
   if (ZZ_pEXArgBound > 0) {
      ...
      sz = sz*ZZ_pE::degree();
      sz = sz + sizeof(vec_ZZ_p);
      //sz = sz + newNTL_VECTOR_HEADER_SIZE + sizeof(vec_ZZ_p);
      sz = sz*F.n;
      //sz = sz + newNTL_VECTOR_HEADER_SIZE + sizeof(vec_ZZ_pE);
      sz = sz + sizeof(vec_ZZ_pE);
      sz = sz/1024;
...

{
   ...
   if (a.length() > n || k < 0 || newNTL_OVERFLOW(k, 1, 0)) 
      Error("ProjectPowers: bad args");
