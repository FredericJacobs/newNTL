...
void BlockConstruct(ZZ_p* x, long n, const ZZ_p&)
{
   if (n <= 0) return; 
...
newNTL_io_vector_impl(ZZ_p,vec_ZZ_p)

newNTL_eq_vector_impl(ZZ_p,vec_ZZ_p)
                  ...
                  long offset)
{
   ...
   if (newNTL_OVERFLOW(offset, 1, 0)) Error("InnerProduct: offset too big");
