...
printf("newNTL_GF2X_ALTCODE ");
#endif


#ifdef newNTL_GF2X_ALTCODE1
printf("newNTL_GF2X_ALTCODE1 ");
#endif


#ifdef newNTL_GF2X_NOINLINE
printf("newNTL_GF2X_NOINLINE ");
#endif
...
int main()
{
   ...
   for (i = 0; i < 10000; i++) {
      GF2X a, b, c, d;
      long da = RandomBnd(5*newNTL_BITS_PER_LONG);
      long db = RandomBnd(5*newNTL_BITS_PER_LONG);
      long dc = RandomBnd(5*newNTL_BITS_PER_LONG);
      long dd = RandomBnd(5*newNTL_BITS_PER_LONG);
   ...
   for (k = 0; k < s; k++) {
      random(a[k], (n + (k % 7))*newNTL_BITS_PER_LONG);
      random(b[k], (n + (k % 8))*newNTL_BITS_PER_LONG);
   }
