...
void xdouble::normalize() 
{
   if (x == 0) 
      ...
      while (x < newNTL_XD_HBOUND_INV) { x *= newNTL_XD_BOUND; e--; }
      while (x > newNTL_XD_HBOUND) { x *= newNTL_XD_BOUND_INV; e++; }
   }
   else {
      while (x > -newNTL_XD_HBOUND_INV) { x *= newNTL_XD_BOUND; e--; }
      while (x < -newNTL_XD_HBOUND) { x *= newNTL_XD_BOUND_INV; e++; }
...
{
   if (a == 0 || a == 1 || (a > 0 && a >= newNTL_XD_HBOUND_INV && a <= newNTL_XD_HBOUND)
       || (a < 0 && a <= -newNTL_XD_HBOUND_INV && a >= -newNTL_XD_HBOUND)) {
...
void conv(double& xx, const xdouble& a)
{
   ...
   while (e > 0) { x *= newNTL_XD_BOUND; e--; }
   while (e < 0) { x *= newNTL_XD_BOUND_INV; e++; }
...
xdouble operator+(const xdouble& a, const xdouble& b)
{
   ...
   if (a.e == b.e) {
      ...
      z.x = a.x + b.x*newNTL_XD_BOUND_INV;
      z.e = a.e;
      z.normalize();
      return z;
   }
      ...
      z.x = a.x*newNTL_XD_BOUND_INV + b.x;
      z.e = b.e;
      z.normalize();
      return z;
   }
...
template<> xdouble to<xdouble>(const ZZ& a)
{
   ...
   if (t == 0) 
      ...
      while (t < newNTL_XD_HBOUND_INV) { t *= newNTL_XD_BOUND; e--; }
      while (t > newNTL_XD_HBOUND) { t *= newNTL_XD_BOUND_INV; e++; }
   }
   else {
      while (t > -newNTL_XD_HBOUND_INV) { t *= newNTL_XD_BOUND; e--; }
      while (t < -newNTL_XD_HBOUND) { t *= newNTL_XD_BOUND_INV; e++; }
   ...
   if (e <= -newNTL_OVFBND)
      Error("xdouble: underflow");
   xdouble res;
   res.x = mpfr_get_d(*t.rep,MPFR_RNDN);
   res.e = e;
   return res;
}
...
{
   staticthread RR t;
   mpfr_set_d(*t.rep,newNTL_XD_BOUND,MPFR_RNDN);
   mpfr_pow_si(*t.rep,*t.rep,a.e,MPFR_RNDN);
   mpfr_mul_d(*t.rep,*t.rep,a.x,MPFR_RNDN);
   conv(x,t);
}
...
{
   staticthread double LogBound = log(newNTL_XD_BOUND);
   if (a.x <= 0) {
...
{
   const double LogBound = log(newNTL_XD_BOUND);
...
long ComputeMax10Power()
{
   ...
   RR::SetPrecision(newNTL_BITS_PER_LONG);

   RR ln2, ln10;
   ComputeLn2(ln2);
   ComputeLn10(ln10);

   long k = to_long( to_RR(newNTL_OVFBND/2) * ln2 / ln10 );
...
xdouble PowerOf10(const ZZ& e)
{
   ...
   staticthread xdouble v10k;
   staticthread long k;

   if (!init) {
      long old_p = RR::precision();
      k = ComputeMax10Power();
      RR::SetPrecision(newNTL_DOUBLE_PRECISION);
...
ostream& operator<<(ostream& s, const xdouble& a)
{
   ...
   log_2_10 = ln10/ln2;
   ZZ log_10_a = to_ZZ(
  (to_RR(a.e)*to_RR(2*newNTL_XD_HBOUND_LOG) + log(fabs(a.x))/log(2.0))/log_2_10);
   ...
   char *bp = newNTL_NEW_OP char[bp_len];
...
template<> xdouble to<xdouble>(const char *s)
{
   long c;
   long cval;
   long sign;
...
template xdouble to<xdouble>(int a);
template xdouble to<xdouble>(long a);
template xdouble to<xdouble>(unsigned int a);
template xdouble to<xdouble>(unsigned long a);
template xdouble to<xdouble>(double a);
template xdouble to<xdouble>(float a);
template xdouble to<xdouble>(const char *a);
template ZZ to<ZZ>(const xdouble& a);
template xdouble to<xdouble>(const ZZ& a);
template double to<double>(const xdouble& a);
template float to<float>(const xdouble& a);
template long to<long>(const xdouble& a);
template int to<int>(const xdouble& a);
template xdouble to<xdouble>(const xdouble& a);
