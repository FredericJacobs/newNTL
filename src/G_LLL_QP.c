...
GivensCache_QP::GivensCache_QP(long m, long n)
{
   ...
   long i;
   buf = newNTL_NEW_OP quad_floatptr[sz]; 
   if (!buf) Error("out of memory");
   for (i = 0; i < sz; i++)
      if (!(buf[i] = newNTL_NEW_OP quad_float[n+1])) Error("out of memory");
...
static void init_red_fudge()
{
   ...
   // initial log_red should be <= newNTL_DOUBLE_PRECISION-2,
   // to help ensure stability in G_BKZ_QP1

   log_red = newNTL_DOUBLE_PRECISION-2; 
           ...
           LLLCheckFct check)
{
   ...
   B1 = newNTL_NEW_OP quad_floatptr[m+1];
   if (!B1) Error("G_LLL_QP: out of memory");

   for (i = 1; i <= m; i++) {
      B1[i] = newNTL_NEW_OP quad_float[n+1];
      if (!B1[i]) Error("G_LLL_QP: out of memory");
   }

   quad_float **mu;
   mu = newNTL_NEW_OP quad_floatptr[m+1];
   if (!mu) Error("G_LLL_QP: out of memory");

   for (i = 1; i <= m; i++) {
      mu[i] = newNTL_NEW_OP quad_float[n+2];
      if (!mu[i]) Error("G_LLL_QP: out of memory");
   }

   quad_float **aux;
   aux = newNTL_NEW_OP quad_floatptr[m+1];
   if (!aux) Error("G_LLL_QP: out of memory");

   for (i = 1; i <= m; i++) {
      aux[i] = newNTL_NEW_OP quad_float[n+1];
         ...
         long beta, long prune, LLLCheckFct check)
{
   ...
   B1 = newNTL_NEW_OP quad_floatptr[m+2];
   ...
   quad_float **mu;
   mu = newNTL_NEW_OP quad_floatptr[m+2];
   ...
   quad_float **aux;
   aux = newNTL_NEW_OP quad_floatptr[m+2];
   ...
   c = newNTL_NEW_OP quad_float[m+2];
   if (!c) Error("G_BKZ_QP: out of memory");

   quad_float cbar;

   quad_float *ctilda;
   ctilda = newNTL_NEW_OP quad_float[m+2];
   if (!ctilda) Error("G_BKZ_QP: out of memory");

   quad_float *vvec;
   vvec = newNTL_NEW_OP quad_float[m+2];
   if (!vvec) Error("G_BKZ_QP: out of memory");

   quad_float *yvec;
   yvec = newNTL_NEW_OP quad_float[m+2];
   if (!yvec) Error("G_BKZ_QP: out of memory");

   quad_float *uvec;
   uvec = newNTL_NEW_OP quad_float[m+2];
   if (!uvec) Error("G_BKZ_QP: out of memory");

   quad_float *utildavec;
   utildavec = newNTL_NEW_OP quad_float[m+2];
         ...
         long beta, long prune, LLLCheckFct check)
{
   ...
   if (!quit && m > 1) {
      ...
      while (z < m-1) {
         ...
         if ((delta-8*red_fudge)*c[jj] > cbar*(1+64/newNTL_FDOUBLE_PRECISION)) {
