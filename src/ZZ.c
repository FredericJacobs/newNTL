...
staticthread long iodigits = 0;
staticthread long ioradix = 0;

// iodigits is the greatest integer such that 10^{iodigits} < newNTL_WSP_BOUND
// ioradix = 10^{iodigits}

namespaceanon void InitZZIO()
{
   long x;

   x = (newNTL_WSP_BOUND-1)/10;
   iodigits = 0;
   ioradix = 1;
...
void _ZZ_local_stack::push(long x)
{
   if (alloc == 0) {
      alloc = 100;
      elts = (long *) newNTL_MALLOC(alloc, sizeof(long), 0);
   }
   ...
   if (top + 1 > alloc) {
      alloc = 2*alloc;
      elts = (long *) newNTL_REALLOC(elts, alloc, sizeof(long), 0);
   }
...
{
   staticthread char *buf = 0;

   if (!buf) {
      buf = (char *) newNTL_MALLOC(iodigits, 1, 0);
...
{
   staticthread ZZ b;
   staticthread _ZZ_local_stack S;
   long r;
   long k;
...
long GCD(long a, long b)
{
   ...
   if (a < 0) {
      if (a < -newNTL_MAX_LONG) Error("GCD: integer overflow");
      a = -a;
   }

   if (b < 0) {
      if (b < -newNTL_MAX_LONG) Error("GCD: integer overflow");
      b = -b;
   }
...
void XGCD(long& d, long& s, long& t, long a, long b)
{
   ...
   if (a < 0) {
      if (a < -newNTL_MAX_LONG) Error("XGCD: integer overflow");
      a = -a;
      aneg = 1;
   }

   if (b < 0) {
      if (b < -newNTL_MAX_LONG) Error("XGCD: integer overflow");
      b = -b;
      bneg = 1;
   }
...
{
   long wn = (bn+newNTL_ZZ_NBITS-1)/newNTL_ZZ_NBITS;
   ...
   if (NumBits(bn) + NumBits(fn) > newNTL_SP_NBITS)
      prime_bnd = newNTL_SP_BOUND;
   else
      prime_bnd = bn*fn;
...
long NextPrime(long m, long NumTrials)
{
   ...
   while (x < newNTL_SP_BOUND && !ProbPrime(x, NumTrials))
      x++;

   if (x >= newNTL_SP_BOUND)
...
long PrimeSeq::next()
{
   ...
   for (;;) {
      ...
      long newshift = pshift + 2*newNTL_PRIME_BND;

      if (newshift > 2 * newNTL_PRIME_BND * (2 * newNTL_PRIME_BND + 1)) {
         /* end of the road */
         exhausted = 1;
         return 0;
      }
...
void PrimeSeq::shift(long newshift)
{
   ...
   if (pshift == 0) {
      ...
      if (!movesieve_mem) {
         movesieve_mem = (char *) newNTL_MALLOC(newNTL_PRIME_BND, 1, 0);
         if (!movesieve_mem) 
            Error("out of memory in PrimeSeq");
      }

      p = movesieve = movesieve_mem;
      for (i = 0; i < newNTL_PRIME_BND; i++)
         p[i] = 1;

      jstep = 3;
      ibound = pshift + 2 * newNTL_PRIME_BND + 1;
      for (i = 0; jstep * jstep <= ibound; i++) {
         if (lowsieve[i]) {
            ...
            for (j = jstart; j < newNTL_PRIME_BND; j += jstep)
               p[j] = 0;
         }
         jstep += 2;
      }
...
void PrimeSeq::start()
{
   ...
   p = lowsieve = (char *) newNTL_MALLOC(newNTL_PRIME_BND, 1, 0);
   if (!p)
      Error("out of memory in PrimeSeq");
   ...
   jstep = 1;
   jstart = -1;
   ibnd = (SqrRoot(2 * newNTL_PRIME_BND + 1) - 3) / 2;
   for (i = 0; i <= ibnd; i++) {
      jstart += 2 * ((jstep += 2) - 1);
      if (p[i])
         for (j = jstart; j < newNTL_PRIME_BND; j += jstep)
...
{
   if (b > (2*newNTL_PRIME_BND+1)*(2*newNTL_PRIME_BND+1)) {
      exhausted = 1;
      return;
   }
   ...
   shift(((b-3) / (2*newNTL_PRIME_BND))* (2*newNTL_PRIME_BND));
...
staticthread long ran_initialized = 0;
staticthread _ZZ_arc4_key ran_key;

staticthread unsigned long default_md5_tab[16] = {
...
void build_arc4_tab(unsigned char *seed_bytes, const ZZ& s)
{
   ...
   txt = newNTL_NEW_OP u_char[nb + 68];
...
{
   unsigned char buf[newNTL_BITS_PER_LONG/8];
   long i;
   unsigned long res;

   ran_bytes(buf, newNTL_BITS_PER_LONG/8);

   res = 0;
   for (i = newNTL_BITS_PER_LONG/8 - 1; i >= 0; i--) {
      res = res << 8;
      res = res | buf[i];
   }
...
void RandomBits(ZZ& x, long l)
{
   ...
   if (newNTL_OVERFLOW(l, 1, 0))
      Error("RandomBits: length too big");

   long nb = (l+7)/8;

   staticthread unsigned char *buf = 0;
   staticthread long buf_len = 0;

   if (nb > buf_len) {
      if (buf) delete [] buf;
      buf_len = ((nb + 1023)/1024)*1024; // allocate in 1024-byte lots
      typedef unsigned char u_char;
      buf = newNTL_NEW_OP u_char[buf_len];
      if (!buf) Error("out of memory");
   }
...
void RandomLen(ZZ& x, long l)
{
   ...
   long nw = (l + newNTL_ZZ_NBITS - 1)/newNTL_ZZ_NBITS;
   x.SetSize(nw);
...
long RandomBnd(long bnd)
{
   ...
   if (l > newNTL_BITS_PER_LONG-2) {
      staticthread ZZ Bnd, res;
...
{
   staticthread double log2 = log(2.0);
   return log(x)/log2;
}
...
{
   const double fudge = (1.0 + 1024.0/newNTL_FDOUBLE_PRECISION);
   ...
   if (9*t > newNTL_FDOUBLE_PRECISION) Error("ErrBoundTest: t too big");
...
long GenPrime_long(long k, long err)
{
   ...
   if (k >= newNTL_BITS_PER_LONG) Error("GenPrime: length too large");
...
template ZZ to<ZZ>(const ZZ& a);
template ZZ to<ZZ>(long a);
template ZZ to<ZZ>(int a);
template ZZ to<ZZ>(unsigned long a);
template ZZ to<ZZ>(unsigned int a);
template ZZ to<ZZ>(const char *s);
template ZZ to<ZZ>(double a);
template ZZ to<ZZ>(float a);
template long to<long>(const ZZ& a);
template int to<int>(const ZZ& a); 
template unsigned long to<unsigned long>(const ZZ& a);
template unsigned int to<unsigned int>(const ZZ& a);
template double to<double>(const ZZ& a);
template float to<float>(const ZZ& a);

void conv(ZZ& res, const mpz_t& x) {
  //TODO improve it
  string s = mpz_class(x).get_str();
  istringstream iss(s); iss >> res; 
}

void conv(mpz_t& u, const ZZ& res) {
  //TODO improve it
  ostringstream oss; oss << res; 
  mpz_set_str(u,oss.str().c_str(),0); 
}
