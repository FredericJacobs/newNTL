...
#if (defined(newNTL_WIZARD_HACK) && defined(newNTL_GF2X_LIB))
#undef newNTL_GF2X_LIB
#endif
...
#define newNTL_GF2X_GCD_CROSSOVER (400L*newNTL_BITS_PER_LONG) 
#define newNTL_GF2X_HalfGCD_CROSSOVER (6L*newNTL_BITS_PER_LONG)
#define newNTL_GF2X_BERMASS_CROSSOVER (200L*newNTL_BITS_PER_LONG)

#else

#define newNTL_GF2X_GCD_CROSSOVER (900L*newNTL_BITS_PER_LONG) 
#define newNTL_GF2X_HalfGCD_CROSSOVER (6L*newNTL_BITS_PER_LONG)
#define newNTL_GF2X_BERMASS_CROSSOVER (450L*newNTL_BITS_PER_LONG)
...
void PlainDivRem(GF2X& q, GF2X& r, const GF2X& a, const GF2X& b)
{
   ...
   sa = a.xrep.length();
   posa = da - newNTL_BITS_PER_LONG*(sa-1);
   sb = b.xrep.length();
   posb = db - newNTL_BITS_PER_LONG*(sb-1);

   dq = da - db;
   sq = dq/newNTL_BITS_PER_LONG + 1;
   posq = dq - newNTL_BITS_PER_LONG*(sq-1);
   ...
   stab.SetLength(newNTL_BITS_PER_LONG);
   long i;

   stab[posb] = b;
   for (i = 1; i <= min(dq, newNTL_BITS_PER_LONG-1); i++) 
      MulByX(stab[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG], 
             stab[((_newntl_ulong)(posb+i-1))%newNTL_BITS_PER_LONG]);

   _newntl_ulong *stab_ptr[newNTL_BITS_PER_LONG];
   long stab_cnt[newNTL_BITS_PER_LONG];

   for (i = 0; i <= min(dq, newNTL_BITS_PER_LONG-1); i++) {
      WordVector& st = stab[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG].xrep;
      long k = st.length();
      stab_ptr[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG] = &st[k-1];
      stab_cnt[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG] = -k+1;
   }

   q.xrep.SetLength(sq);
   _newntl_ulong *qp = q.xrep.elts();
   for (i = 0; i < sq; i++)
      qp[i] = 0;

   _newntl_ulong *atop = &ap[sa-1];
   _newntl_ulong *qtop = &qp[sq-1];
   _newntl_ulong *stab_top;
   ...
   while (1) {
      ...
      posa--;
      if (posa < 0) {
         posa = newNTL_BITS_PER_LONG-1;
         atop--;
      }

      posq--;
      if (posq < 0) {
         posq = newNTL_BITS_PER_LONG-1;
         qtop--;
      }
   }
...
void PlainRem(GF2X& r, const GF2X& a, const GF2X& b)
{
   ...
   stab[posb] = b;
   for (i = 1; i <= min(da-db, newNTL_BITS_PER_LONG-1); i++) 
      MulByX(stab[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG], 
             stab[((_newntl_ulong)(posb+i-1))%newNTL_BITS_PER_LONG]);
   ...
   for (i = 0; i <= min(da-db, newNTL_BITS_PER_LONG-1); i++) {
      WordVector& st = stab[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG].xrep;
      long k = st.length();
      stab_ptr[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG] = &st[k-1];
      stab_cnt[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG] = -k+1;
...
static _newntl_ulong invtab[128] = {
...
void NewtonInvTrunc(GF2X& c, const GF2X& a, long e)
{
   ...
   g.xrep.SetMaxLength((E[0]+newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG + 1);
   g0.xrep.SetMaxLength((E[0]+newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG + 1);
   g1.xrep.SetMaxLength(((3*E[0]+1)/2+newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG+1);
   g2.xrep.SetMaxLength((E[0]+newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG + 1);
...
void SparsityCheck(const GF2X& f, long& k3, long& k2, long& k1)
{
   ...
   if (n-t < newNTL_BITS_PER_LONG || t > (n+1)/2) {
      k3 = 0;
      return;
   }
...
void build(GF2XModulus& F, const GF2X& f)
{
   ...
   long sb = F.sn;
   long posb = n - newNTL_BITS_PER_LONG*(sb-1);
   ...
   if (F.sn > 1 && deg_f0 < newNTL_BITS_PER_LONG 
       && deg_f0 >= newNTL_BITS_PER_LONG/2) {
      if (F.size >= 6)
         F.method = GF2X_MOD_MUL;
      else
         F.method = GF2X_MOD_SPECIAL;
   }
   else if (F.sn > 1 && deg_f0 < newNTL_BITS_PER_LONG/2) {
      if (F.size >= 4)
         F.method = GF2X_MOD_MUL;
      else
         F.method = GF2X_MOD_SPECIAL;
   ...
   if (F.method == GF2X_MOD_SPECIAL) {
      if (!F.stab_cnt) F.stab_cnt = newNTL_NEW_OP long[newNTL_BITS_PER_LONG];
      long *stab_cnt = F.stab_cnt;
      if (!stab_cnt) Error("out of memory");

      if (!F.stab1) F.stab1 = newNTL_NEW_OP _newntl_ulong[2*newNTL_BITS_PER_LONG];
      _newntl_ulong *stab1 = F.stab1;
      ...
      for (i = 1; i < newNTL_BITS_PER_LONG; i++) {
         long kk0 = ((_newntl_ulong)(posb+i-1))%newNTL_BITS_PER_LONG;
         long kk1 = ((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG;

         stab1[kk1<<1] = stab1[kk0<<1] << 1;
         stab1[(kk1<<1)+1] = (stab1[(kk0<<1)+1] << 1) 
                          | (stab1[kk0<<1] >> (newNTL_BITS_PER_LONG-1));
      ...
      if (!F.stab_ptr) F.stab_ptr = newNTL_NEW_OP _newntl_ulong_ptr[newNTL_BITS_PER_LONG];
      _newntl_ulong **stab_ptr = F.stab_ptr;
      ...
      for (i = 0; i < newNTL_BITS_PER_LONG; i++) {
         WordVector& st = stab[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG].xrep;
         long k = st.length();
         stab_ptr[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG] = &st[k-1];
         stab_cnt[((_newntl_ulong)(posb+i))%newNTL_BITS_PER_LONG] = -k+1;
   ...
   stab_cnt(0), stab_ptr(0), stab1(0), tracevec(F.tracevec)
{
   if (method == GF2X_MOD_SPECIAL) {
      long i;
      stab1 = newNTL_NEW_OP _newntl_ulong[2*newNTL_BITS_PER_LONG];
      if (!stab1) Error("GF2XModulus: out of memory");
      for (i = 0; i < 2*newNTL_BITS_PER_LONG; i++)
         stab1[i] = F.stab1[i];
      stab_cnt = newNTL_NEW_OP long[newNTL_BITS_PER_LONG];
      ...
      if (F.stab_ptr) {
         stab_ptr = newNTL_NEW_OP _newntl_ulong_ptr[newNTL_BITS_PER_LONG];
         if (!stab_ptr) Error("GF2XModulus: out of memory");
      
         for (i = 0; i < newNTL_BITS_PER_LONG; i++) {
            WordVector& st = stab[((_newntl_ulong)(posn+i))%newNTL_BITS_PER_LONG].xrep;
            long k = st.length();
            stab_ptr[((_newntl_ulong)(posn+i))%newNTL_BITS_PER_LONG] = &st[k-1];
            stab_cnt[((_newntl_ulong)(posn+i))%newNTL_BITS_PER_LONG] = -k+1;
         }
      }
   }
}
...
GF2XModulus& GF2XModulus::operator=(const GF2XModulus& F)
{
   ...
   if (method == GF2X_MOD_SPECIAL) {
      long i;
      if (!stab1) stab1 = newNTL_NEW_OP _newntl_ulong[2*newNTL_BITS_PER_LONG];
      if (!stab1) Error("GF2XModulus: out of memory");
      for (i = 0; i < 2*newNTL_BITS_PER_LONG; i++)
         stab1[i] = F.stab1[i];
      if (!stab_cnt) stab_cnt = newNTL_NEW_OP long[newNTL_BITS_PER_LONG];
      if (!stab_cnt) Error("GF2XModulus: out of memory");
      for (i = 0; i < newNTL_BITS_PER_LONG; i++)
         stab_cnt[i] = F.stab_cnt[i];
   }
      ...
      if (F.stab_ptr) {
         if (!stab_ptr) stab_ptr = newNTL_NEW_OP _newntl_ulong_ptr[newNTL_BITS_PER_LONG];
         if (!stab_ptr) Error("GF2XModulus: out of memory");
...
void TrinomReduce(GF2X& x, const GF2X& a, long n, long k)
{
   ...
   long wdiff = (n-k)/newNTL_BITS_PER_LONG;
   long bdiff = (n-k) - wdiff*newNTL_BITS_PER_LONG;
   ...
   if (bn == 0) {
      if (bdiff == 0) {
         ...
         while (m >= wn) {
            ...
            pp = &p[m-wdiff];
            *pp ^= (w >> bdiff);
            *(pp-1) ^= (w << (newNTL_BITS_PER_LONG-bdiff));
            p[m-wn] ^= w;
            m--;
         }
      }
      ...
      if (bdiff == 0) {
         ...
         while (m > wn) {
            ...
            pp = &p[m-wn];
            *pp ^= (w >> bn);
            *(pp-1) ^= (w << (newNTL_BITS_PER_LONG-bn));
            m--;
         }
         ...
         p[m-wdiff] ^= (w >> bdiff);
         if (m-wdiff-1 >= 0) p[m-wdiff-1] ^= (w << (newNTL_BITS_PER_LONG-bdiff));
         p[0] ^= (w >> bn);
         p[m] &= ((1UL<<bn)-1UL); 
      }
   }
...
void PentReduce(GF2X& x, const GF2X& a, long n, long k3, long k2, long k1)
{
   ...
   long wdiff1 = (n-k1)/newNTL_BITS_PER_LONG;
   long bdiff1 = (n-k1) - wdiff1*newNTL_BITS_PER_LONG;

   long wdiff2 = (n-k2)/newNTL_BITS_PER_LONG;
   long bdiff2 = (n-k2) - wdiff2*newNTL_BITS_PER_LONG;

   long wdiff3 = (n-k3)/newNTL_BITS_PER_LONG;
   long bdiff3 = (n-k3) - wdiff3*newNTL_BITS_PER_LONG;
   ...
   while (m > wn) {
      ...
      if (bdiff1 == 0) 
         ...
         pp = &p[m-wdiff1];
         *pp ^= (w >> bdiff1);
         *(pp-1) ^= (w << (newNTL_BITS_PER_LONG-bdiff1));
      }
      ...
      if (bdiff2 == 0) 
         ...
         pp = &p[m-wdiff2];
         *pp ^= (w >> bdiff2);
         *(pp-1) ^= (w << (newNTL_BITS_PER_LONG-bdiff2));
      }
      ...
      if (bdiff3 == 0) 
         ...
         pp = &p[m-wdiff3];
         *pp ^= (w >> bdiff3);
         *(pp-1) ^= (w << (newNTL_BITS_PER_LONG-bdiff3));
      }
   ...
   if (bdiff1 == 0)
      ...
      p[m-wdiff1] ^= (w >> bdiff1);
      if (m-wdiff1-1 >= 0) p[m-wdiff1-1] ^= (w << (newNTL_BITS_PER_LONG-bdiff1));
   }
   ...
   if (bdiff2 == 0)
      ...
      p[m-wdiff2] ^= (w >> bdiff2);
      if (m-wdiff2-1 >= 0) p[m-wdiff2-1] ^= (w << (newNTL_BITS_PER_LONG-bdiff2));
   }
   ...
   if (bdiff3 == 0)
      ...
      p[m-wdiff3] ^= (w >> bdiff3);
      if (m-wdiff3-1 >= 0) p[m-wdiff3-1] ^= (w << (newNTL_BITS_PER_LONG-bdiff3));
   }
...
void RightShiftAdd(GF2X& c, const GF2X& a, long n)
{
   ...
   _newntl_ulong *cp = c.xrep.elts();
   const _newntl_ulong *ap = a.xrep.elts();
...
void rem(GF2X& r, const GF2X& a, const GF2XModulus& F)
{
   ...
   if (da < n) {
      ...
      long sa = a.xrep.length();
      long posa = da - newNTL_BITS_PER_LONG*(sa-1);
...
void DivRem(GF2X& q, GF2X& r, const GF2X& a, const GF2XModulus& F)
{
   ...
   if (da < n) {
      ...
      long dq = da - n;
      long sq = dq/newNTL_BITS_PER_LONG + 1;
      long posq = dq - newNTL_BITS_PER_LONG*(sq-1);
...
static inline 
void swap(_newntl_ulong_ptr& a, _newntl_ulong_ptr& b)  
{  _newntl_ulong_ptr t;  t = a; a = b; b = t; }
...
void BaseGCD(GF2X& d, const GF2X& a_in, const GF2X& b_in)
{
   ...
   _newntl_ulong *ap = a.xrep.elts();
   _newntl_ulong *bp = b.xrep.elts();

   long da = deg(a);
   long wa = da/newNTL_BITS_PER_LONG;
   long ba = da - wa*newNTL_BITS_PER_LONG;

   long db = deg(b);
   long wb = db/newNTL_BITS_PER_LONG;
   long bb = db - wb*newNTL_BITS_PER_LONG;
   ...
   for (;;) {
      ...
      _newntl_ulong msk = 1UL << ba;
      _newntl_ulong aa = ap[wa];
      ...
      while ((aa & msk) == 0) {
         ...
         if (!msk) {
            wa--;
            ba = newNTL_BITS_PER_LONG-1;
            msk = 1UL << (newNTL_BITS_PER_LONG-1);
            if (wa < 0) break;
            aa = ap[wa];
         }
      }
...
#define XX_STEP(ap,da,wa,ba,rp,sr,bp,db,wb,bb,sp,ss)  \
  ...
  \
      if (delta == 0) {  \
         ...
         long i; \
         _newntl_ulong tt, tt1;  \
  \
         tt = bp[wb] >> (newNTL_BITS_PER_LONG-1);  \
         if (tt) ap[wb+1] ^= tt;  \
         tt = bp[wb];  \
         for (i = wb; i >= 1; i--)  \
            tt1 = bp[i-1], ap[i] ^= (tt << 1) | (tt1 >> (newNTL_BITS_PER_LONG-1)),  \
            tt = tt1; \
         ap[0] ^= tt << 1;  \
  \
         if (ss > 0) {  \
            long t = ss; \
            tt = sp[ss-1] >> (newNTL_BITS_PER_LONG-1);  \
            if (tt) rp[ss] ^= tt, t++;  \
            tt = sp[ss-1]; \
            for (i = ss-1; i >= 1; i--)  \
               tt1=sp[i-1],  \
               rp[i] ^= (tt << 1) | (tt1 >> (newNTL_BITS_PER_LONG-1)),  \
               tt = tt1; \
            rp[0] ^= tt << 1;  \
            if (t > sr) sr = t; \
         }  \
      }  \
      else if (delta < newNTL_BITS_PER_LONG) {  \
         long i; \
         _newntl_ulong tt, tt1;  \
         long rdelta = newNTL_BITS_PER_LONG-delta; \
  \
         tt = bp[wb] >> rdelta;  \
         if (tt) ap[wb+1] ^= tt;  \
         tt=bp[wb]; \
  ...
  \
         ...
         long t = ss + (da-db+newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG;  \
         if (t > sr) {  \
...
void BaseInvMod(GF2X& d, GF2X& s, const GF2X& a, const GF2X& f)
{
   ...
   if ((sa >= 10 && 2*sf > 3*sa) || 
       sf > newNTL_GF2X_GCD_CROSSOVER/newNTL_BITS_PER_LONG) {
      GF2XRegister(t);
...
void VectorCopy(vec_GF2& x, const GF2X& a, long n)
{
   ...
   if (newNTL_OVERFLOW(n, 1, 0))
      Error("overflow in VectorCopy");

   long wa = a.xrep.length();
   long wx = (n + newNTL_BITS_PER_LONG - 1)/newNTL_BITS_PER_LONG;
                   ...
                   const vec_GF2X& H, long n, WordVector& t)
{
   ...
   _newntl_ulong *tp = t.elts();

   for (i = 0; i < n; i++)
      tp[i] = 0;


   long w_low = low/newNTL_BITS_PER_LONG;
   long b_low = low - w_low*newNTL_BITS_PER_LONG;

   
   const _newntl_ulong *vp = &v.xrep[w_low];
   _newntl_ulong msk = 1UL << b_low;
   _newntl_ulong vv = *vp;
                   ...
                   const GF2XModulus& F)
{
   ...
   if (deg(a) >= n || k < 0 || newNTL_OVERFLOW(k, 1, 0)) 
...
{
   if (m < 0 || newNTL_OVERFLOW(m, 1, 0)) Error("MinPoly: bad args");
...
class _newNTL_GF2XMatrix {
private:

   _newNTL_GF2XMatrix(const _newNTL_GF2XMatrix&);  // disable
   GF2X elts[2][2];
...
static
void mul(GF2X& U, GF2X& V, const _newNTL_GF2XMatrix& M)
// (U, V)^T = M*(U, V)^T
{
   GF2X t1, t2, t3;
...
static
void mul(_newNTL_GF2XMatrix& A, _newNTL_GF2XMatrix& B, _newNTL_GF2XMatrix& C)
...
static
void IterHalfGCD(_newNTL_GF2XMatrix& M_out, GF2X& U, GF2X& V, long d_red)
{
...
static
void HalfGCD(_newNTL_GF2XMatrix& M_out, const GF2X& U, const GF2X& V, long d_red)
{
   ...
   if (d_red <= newNTL_GF2X_HalfGCD_CROSSOVER) {
...
void GCD(GF2X& d, const GF2X& u, const GF2X& v)
{
   ...
   if (su <= newNTL_GF2X_GCD_CROSSOVER/newNTL_BITS_PER_LONG &&
       sv <= newNTL_GF2X_GCD_CROSSOVER/newNTL_BITS_PER_LONG) {
      OldGCD(d, u, v);
      return;
   }
   ...
   while (du1 >= newNTL_GF2X_GCD_CROSSOVER && !IsZero(v1)) {
      HalfGCD(u1, v1);
...
static
void XHalfGCD(_newNTL_GF2XMatrix& M_out, GF2X& U, GF2X& V, long d_red)
{
   if (IsZero(V) || deg(V) <= deg(U) - d_red) {
      set(M_out(0,0));   clear(M_out(0,1));
      clear(M_out(1,0)); set(M_out(1,1));
...
void XGCD(GF2X& d, GF2X& s, GF2X& t, const GF2X& a, const GF2X& b)
{
   ...
   if (sa <= newNTL_GF2X_GCD_CROSSOVER/newNTL_BITS_PER_LONG &&
       sb <= newNTL_GF2X_GCD_CROSSOVER/newNTL_BITS_PER_LONG) {
      OldXGCD(d, s, t, a, b);
      return;
   }
...
{  
   if (m < newNTL_GF2X_BERMASS_CROSSOVER) {
