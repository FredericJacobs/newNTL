...
{
   staticthread long m = newNTL_FFTMaxRootBnd + 1;
   staticthread long k = 0;
   ...
   for (;;) {
      if (k == 0) {
         ...
         k = 1L << (newNTL_SP_NBITS-m-2);
      }

      k--;

      cand = (1L << (newNTL_SP_NBITS-1)) + (k << (m+1)) + (1L << m) + 1;
...
long CalcMaxRoot(long p)
{
   ...
   if (k > newNTL_FFTMaxRoot)
      return newNTL_FFTMaxRoot;
   else
      return k; 
}
...
void UseFFTPrime(long index)
{
   ...
   if (index == 0) { 
      FFTPrime = (long *) newNTL_MALLOC(100, sizeof(long), 0);
      RootTable = (long **) newNTL_MALLOC(100, sizeof(long *), 0);
      RootInvTable = (long **) newNTL_MALLOC(100, sizeof(long *), 0);
      TwoInvTable = (long **) newNTL_MALLOC(100, sizeof(long *), 0);
      FFTPrimeInv = (double *) newNTL_MALLOC(100, sizeof(double), 0);
   }
   else if ((index % 100) == 0) {
      FFTPrime = (long *) newNTL_REALLOC(FFTPrime, index+100, sizeof(long), 0);
      RootTable = (long **) 
                  newNTL_REALLOC(RootTable, index+100, sizeof(long *), 0);
      RootInvTable = (long **) 
                     newNTL_REALLOC(RootInvTable, index+100, sizeof(long *), 0);
      TwoInvTable = (long **) 
                    newNTL_REALLOC(TwoInvTable, index+100, sizeof(long *), 0);
      FFTPrimeInv = (double *) 
                    newNTL_REALLOC(FFTPrimeInv, index+100, sizeof(double), 0);
   }
   ...
   if (!(rt = RootTable[index] = (long*) newNTL_MALLOC(mr+1, sizeof(long), 0)))
      Error("out of space");
   if (!(rit = RootInvTable[index] = (long*) newNTL_MALLOC(mr+1, sizeof(long), 0)))
      Error("out of space");
   if (!(tit = TwoInvTable[index] = (long*) newNTL_MALLOC(mr+1, sizeof(long), 0)))
      Error("out of space");
...
{
   staticthread long* mem[newNTL_FFTMaxRoot+1];
   ...
   rev = mem[k];
   if (!rev) {
      rev = mem[k] = (long *) newNTL_MALLOC(n, sizeof(long), 0);
...

{
   ...
   staticthread long tab_size = 0;
   staticthread long *wtab = 0;
   staticthread mulmod_precon_t *wqinvtab = 0;

   if (!tab_size) {
      tab_size = k;

      wtab = (long *) newNTL_MALLOC(1L << (k-2), sizeof(long), 0);
      wqinvtab = (mulmod_precon_t *) 
                 newNTL_MALLOC(1L << (k-2), sizeof(mulmod_precon_t), 0);
      if (!wtab || !wqinvtab) Error("out of space");
   }
   else if (tab_size < k) {
      tab_size = k;

      wtab = (long *) newNTL_REALLOC(wtab, 1L << (k-2), sizeof(long), 0);
      wqinvtab = (mulmod_precon_t *) 
                 newNTL_REALLOC(wqinvtab, 1L << (k-2), sizeof(mulmod_precon_t), 0);
      if (!wtab || !wqinvtab) Error("out of space");
   }
