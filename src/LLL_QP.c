               ...
               quad_float *c, long k, double bound, long st, quad_float *buf)
{
   ...
   for (j = st; j <= k-1; j++) {
      if (b[k].hi/newNTL_FDOUBLE_PRECISION < newNTL_FDOUBLE_PRECISION/b[j].hi) {
           ...
           long m, long init_k, long &quit)
{
   ...
   if (bound == 0) {
      ...
      bound = 1;
      for (i = 2*long(0.15*2*newNTL_DOUBLE_PRECISION); i > 0; i--) {
         bound = bound * 2;
      }
   }
   ...
   quad_float *buf;
   buf = newNTL_NEW_OP quad_float [m+1];
         ...
         long beta, long prune, LLLCheckFct check)
{
   ...
   b = newNTL_NEW_OP quad_float[m+2];
