...
void vec_GF2::SetLength(long n)
{
   ...
   long wdlen = (n+newNTL_BITS_PER_LONG-1)/newNTL_BITS_PER_LONG;

   if (n < len) {
      // have to clear bits n..len-1

      long q = n/newNTL_BITS_PER_LONG;
      long p = n - q*newNTL_BITS_PER_LONG;

      _newntl_ulong *x = rep.elts();

      x[q] &= (1UL << p) - 1UL;

      long q1 = (len-1)/newNTL_BITS_PER_LONG;
      long i;
...
vec_GF2& vec_GF2::operator=(const vec_GF2& a)
{
   ...
   _newntl_ulong *x = rep.elts();
   const _newntl_ulong *y = a.rep.elts();
...
GF2 vec_GF2::get(long i) const
{
   ...
   long q = i/newNTL_BITS_PER_LONG;
   long p = i - q*newNTL_BITS_PER_LONG;
...
void add(vec_GF2& x, const vec_GF2& a, const vec_GF2& b)
{
   ...
   const _newntl_ulong *ap = a.rep.elts();
   const _newntl_ulong *bp = b.rep.elts();
...
long IsZero(const vec_GF2& x)
{
   long wlen = x.rep.length();
   long i;
   const _newntl_ulong *xp = x.rep.elts();
...

{
   ...
   if (bn == 0) {
      ...
      for (i = sa-1; i >= wn+1; i--)
         xp[i] = (ap[i-wn] << bn) | (ap[i-wn-1] >> (newNTL_BITS_PER_LONG-bn));
      xp[wn] = ap[0] << bn;
      for (i = wn-1; i >= 0; i--)
         xp[i] = 0;
   }

   long p = l % newNTL_BITS_PER_LONG;
...
static _newntl_ulong revtab[256] = {
...
void VectorCopy(vec_GF2& x, const vec_GF2& a, long n)
{
   ...
   if (newNTL_OVERFLOW(n, 1, 0)) Error("overflow in VectorCopy");

   long m = min(n, a.length());

   x.SetLength(n);

   long wn = (n + newNTL_BITS_PER_LONG - 1)/newNTL_BITS_PER_LONG;
   long wm = (m + newNTL_BITS_PER_LONG - 1)/newNTL_BITS_PER_LONG;
