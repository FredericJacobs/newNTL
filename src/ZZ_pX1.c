...
void HalfGCD(ZZ_pXMatrix& M_out, const ZZ_pX& U, const ZZ_pX& V, long d_red)
{
   ...
   if (d_red <= newNTL_ZZ_pX_HalfGCD_CROSSOVER) {
...
void GCD(ZZ_pX& d, const ZZ_pX& u, const ZZ_pX& v)
{
   ...
   while (deg(u1) > newNTL_ZZ_pX_GCD_CROSSOVER && !IsZero(v1)) {
      HalfGCD(u1, v1);
...
void BuildFromRoots(ZZ_pX& x, const vec_ZZ_p& a)
{
   ...
   long k0 = NextPowerOfTwo(newNTL_ZZ_pX_FFT_CROSSOVER);
...
newNTL_vector_impl(ZZ_pX,vec_ZZ_pX)

newNTL_eq_vector_impl(ZZ_pX,vec_ZZ_pX)

newNTL_io_vector_impl(ZZ_pX,vec_ZZ_pX)
...
void MinPolySeq(ZZ_pX& h, const vec_ZZ_p& a, long m)
{
   ...
   if (m > newNTL_ZZ_pX_BERMASS_CROSSOVER)
      GCDMinPolySeq(h, a, m);
   else
...
{
   if (deg(f) <= newNTL_ZZ_pX_TRACE_CROSSOVER)
      PlainTraceVec(S, f);
   else
      FastTraceVec(S, f);
}
...
{
   if (deg(u) <= newNTL_ZZ_pX_GCD_CROSSOVER || deg(v) <= newNTL_ZZ_pX_GCD_CROSSOVER) { 
