...

{
   ...
   for (j = st; j <= k-1; j++) {
      ...
      // test = b[k]*b[j] >= newNTL_FDOUBLE_PRECISION^2

      test = (b[k]/newNTL_FDOUBLE_PRECISION >= newNTL_FDOUBLE_PRECISION/b[j]);
           ...
           long m, long init_k, long &quit)
{
   ...
   if (bound == 0) {
      ...
      bound = 1;
      for (i = 2*long(0.15*newNTL_DOUBLE_PRECISION); i > 0; i--)
         bound = bound * 2;
   }
