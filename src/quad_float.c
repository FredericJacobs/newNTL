...
#if (defined(__GNUC__) && newNTL_EXT_DOUBLE && (defined(__i386__) || defined(__i486__) || defined(__i586__)))

#if (!defined(newNTL_X86_FIX) && !defined(newNTL_NO_X86_FIX))
...
#if (newNTL_EXT_DOUBLE && !defined(newNTL_X86_FIX))
...
template<> quad_float to_quad_float(long n)
{
START_FIX
   DOUBLE xhi, xlo;
   DOUBLE u, v;
...
template<> quad_float to_quad_float(unsigned long n)
{
START_FIX
   DOUBLE xhi, xlo, t;
   DOUBLE u, v;

   const double bnd = double(1L << (newNTL_BITS_PER_LONG-2))*4.0;
...
START_FIX
  ...
  C = newNTL_QUAD_FLOAT_SPLIT*x.hi;
  hx = C-x.hi;
  c = newNTL_QUAD_FLOAT_SPLIT*y.hi;
  hx = C-hx;
  tx = x.hi-hx;
  hy = c-y.hi;
  C = x.hi*y.hi;
...
START_FIX
  ...
  C = x.hi/y.hi;
  c = newNTL_QUAD_FLOAT_SPLIT*C;
  hc = c-C;
  u = newNTL_QUAD_FLOAT_SPLIT*y.hi;
  hc = c-hc;
  tc = C-hc;
  hy = u-y.hi;
  U = C * y.hi;
...
{
   z.hi = _newntl_ldexp(1.0, e);
   z.lo = 0;
}


template<> long to<long>(const quad_float& x)
{
   double fhi, flo;
...
void conv(quad_float& z, const ZZ& a)
{
   ...
   if (fabs(z.hi) < newNTL_FDOUBLE_PRECISION && z.lo != 0)
...
{
   long old_p = RR::precision();
   RR::SetPrecision(4*newNTL_DOUBLE_PRECISION);
...
END_FIX
   return k == newNTL_DOUBLE_PRECISION;
}
...
quad_float ldexp(const quad_float& x, long exp) { // x*2^exp
   ...
   xhi = _newntl_ldexp(x.hi, exp);
   xlo = _newntl_ldexp(x.lo, exp);
