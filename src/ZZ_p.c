...
ZZ_pInfoT::ZZ_pInfoT(const ZZ& NewP)
{
   ...
   ExtendedModulusSize = 2*size + 
                 (newNTL_BITS_PER_LONG + newNTL_ZZ_NBITS - 1)/newNTL_ZZ_NBITS;
...
void ZZ_pInfoT::init()
{
   ...
   LeftShift(B, B, newNTL_FFTMaxRoot+newNTL_FFTFudge);
   ...
   if (8.0*fn*(fn+32) > newNTL_FDOUBLE_PRECISION)
      Error("modulus too big");


   if (8.0*fn*(fn+32) > newNTL_FDOUBLE_PRECISION/double(newNTL_SP_BOUND))
      QuickCRT = 0;
   else
      QuickCRT = 1;
...
void CopyPointer(ZZ_pInfoPtr& dst, ZZ_pInfoPtr src)
{
   ...
   if (src) {
      if (src->ref_count == newNTL_MAX_LONG)
...
ZZ_p& ZZ_pTemp::val() const
{
   if (!ZZ_pInfo->temps[pos]) 
      ZZ_pInfo->temps[pos] = newNTL_NEW_OP ZZ_p;
