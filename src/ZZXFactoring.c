...

{
   ...
   if (k < 2 || e < 1 || newNTL_OVERFLOW(e, 1, 0)) Error("MultiLift: bad args");
...

{
   ...
   for (; NumPrimes < ZZXFac_InitNumPrimes;) {
      ...
      if (r < minr) {
         minr = r;
         delete bestfac;
         bestfac = newNTL_NEW_OP vec_pair_zz_pX_long;
         *bestfac = thisfac;
         delete besth;
         besth = newNTL_NEW_OP zz_pX;
         *besth = thish;
         bestp.save();
   ...
   if (!irred) {
      ...
      spfactors = newNTL_NEW_OP vec_zz_pX;
...
const int ZZX_OVERLIFT = newNTL_BITS_PER_LONG;
  // number of bits by which we "overlift"....this enables, in particular,
  // the "n-1" test.  
  // Must lie in the range 4..newNTL_BITS_PER_LONG.
             ...
             unsigned long thresh1, long **shamt_tab, long pruning)
{
   ...
   if (pruning) {
      for (i = 2; i <= pruning; i++) {
         long len = min(k-1, i);
         for (j = 2; j <= len; j++) {
            long ub = (((1L << (newNTL_BITS_PER_LONG-shamt_tab[i][j])) 
                      + TBL_MSK) >> TBL_SHAMT); 
            for (t = 0; t < ub; t++)
               lookup_tab[i][j][t] = 0;
         }
                ...
                long **shamt_tab)
{
   ...
   for (i = 0; i < r; i++) {
      ...
      LeftShift(aa, rep(a), newNTL_BITS_PER_LONG);
      div(aa, aa, p);
      ratio[i] = to_ulong(aa);
   }
                     ...
                     vec_ulong& sum_stack_vec, long& SumLen)
{
   ...
   if (SumLen == 0) {
      unsigned long epsilon = (1UL << (newNTL_BITS_PER_LONG-ZZX_OVERLIFT));
...
long shamt_tab_init(long pos, long card, long pruning, long thresh1_len)
{
   ...
   t = min(t, newNTL_BITS_PER_LONG-thresh1_len);


   return newNTL_BITS_PER_LONG-t;
}
                       ...
                       long verbose)
{
   ...
   if (NumBits(k) > newNTL_BITS_PER_LONG/2-2)
   ...
   pruning = min(pruning, newNTL_BITS_PER_LONG-EXTRA_BITS-thresh1_len);
   ...
   if (pruning) {
      ...
      shamt_tab = newNTL_NEW_OP long_p[pruning+1];
      if (!shamt_tab) Error("out of mem");
      shamt_tab[0] = shamt_tab[1] = 0;

      for (i = 2; i <= pruning; i++) {
         long len = min(k-1, i);
         shamt_tab[i] = newNTL_NEW_OP long[len+1];
      ...
      lookup_tab = newNTL_NEW_OP TBL_T_pp[pruning+1];
      ...
      for (i = 2; i <= pruning; i++) {
         long len = min(k-1, i);
         lookup_tab[i] = newNTL_NEW_OP TBL_T_p[len+1];
         if (!lookup_tab[i]) Error("out of mem");

         lookup_tab[i][0] = lookup_tab[i][1] = 0;

         for (j = 2; j <= len; j++) {
            lookup_tab[i][j] = newNTL_NEW_OP TBL_T[((1L << (newNTL_BITS_PER_LONG-shamt_tab[i][j]))+TBL_MSK) >> TBL_SHAMT];
...
void gauss(ZZ& d_out, mat_ZZ& R_out, const mat_ZZ& M)
{
   ...
   for (;;) {
      long p = GenPrime_long(newNTL_SP_NBITS);
      zz_p::init(p);
