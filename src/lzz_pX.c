...
#define QUICK_CRT (newNTL_DOUBLE_PRECISION - newNTL_SP_NBITS > 12)
...
void mul(zz_pX& x, const zz_pX& a, const zz_pX& b)
{
   ...
   if (deg(a) > newNTL_zz_pX_MUL_CROSSOVER && deg(b) > newNTL_zz_pX_MUL_CROSSOVER)
      FFTMul(x, a, b);
   else
      PlainMul(x, a, b);
}
...
{
   if (deg(a) > newNTL_zz_pX_MUL_CROSSOVER)
      FFTSqr(x, a);
   else
      PlainSqr(x, a);
}
...
void PlainMul(zz_pX& c, const zz_pX& a, const zz_pX& b)
{
   ...
   long p = zz_p::modulus();
   long use_FP = ((p < newNTL_SP_BOUND/KARX) && 
                 (double(p)*double(p) < newNTL_FDOUBLE_PRECISION/KARX));
...
void PlainSqr(zz_pX& c, const zz_pX& a)
{
   ...
   long p = zz_p::modulus();
   long use_FP = ((p < newNTL_SP_BOUND/KARSX) && 
                 (double(p)*double(p) < newNTL_FDOUBLE_PRECISION/KARSX));
...
{
   if (NewK < -1 || NewK >= newNTL_BITS_PER_LONG-1)
   ...
   for (i = 0; i < zz_pInfo->NumPrimes; i++) {
      if ( !(tbl[i] = (long *) newNTL_MALLOC(n, sizeof(long), 0)) )
...
// a bit more robust.  
   ...
   for (i = 0; i < n; i++) {
      ...
      q = q + Q;
#if (newNTL_BITS_PER_LONG - newNTL_SP_NBITS <= 4)
...
void rem21(zz_pX& x, const zz_pX& a, const zz_pXModulus& F)
{
   ...
   if (!F.UseFFT || da - n <= newNTL_zz_pX_MOD_CROSSOVER) {
      PlainRem(x, a, F.f);
      return;
   }
...
void rem(zz_pX& x, const zz_pX& a, const zz_pXModulus& F)
{
   ...
   else if (!F.UseFFT || da-n <= newNTL_zz_pX_MOD_CROSSOVER) {
      PlainRem(x, a, F.f);
      return;
   }
...
void MulMod(zz_pX& x, const zz_pX& a, const zz_pX& b, const zz_pXModulus& F)
{
   ...
   if (!F.UseFFT || da <= newNTL_zz_pX_MUL_CROSSOVER || db <= newNTL_zz_pX_MUL_CROSSOVER) {
      zz_pX P1;
      mul(P1, a, b);
      rem(x, P1, F);
      return;
...
void SqrMod(zz_pX& x, const zz_pX& a, const zz_pXModulus& F)
{
   ...
   if (!F.UseFFT || da <= newNTL_zz_pX_MUL_CROSSOVER) {
      zz_pX P1;
      sqr(P1, a);
      rem(x, P1, F);
      return;
...
void InvTrunc(zz_pX& x, const zz_pX& a, long m)
{
   ...
   if (newNTL_OVERFLOW(m, 1, 0))
      Error("overflow in InvTrunc");

   if (&x == &a) {
      zz_pX la;
      la = a;
      if (m > newNTL_zz_pX_NEWTON_CROSSOVER && deg(a) > 0)
...
void build(zz_pXModulus& x, const zz_pX& f)
{
   ...
   if (x.n <= newNTL_zz_pX_MOD_CROSSOVER + 1) {
      x.UseFFT = 0;
      return;
   }
                         ...
                         const zz_pXModulus& F)
{
   ...
   if (!F.UseFFT || db <= newNTL_zz_pX_MOD_CROSSOVER) {
      x.UseFFT = 0;
      return;
   }
                                      ...
                                      const zz_pXModulus& F)
{
   ...
   if (!B.UseFFT || !F.UseFFT || da <= newNTL_zz_pX_MOD_CROSSOVER) {
      zz_pX P1;
      mul(P1, a, B.b);
      rem(x, P1, F);
      return;
...
void NewtonInvTrunc(zz_pX& x, const zz_pX& a, long m)
{
   ...
   long log2_newton = NextPowerOfTwo(newNTL_zz_pX_NEWTON_CROSSOVER)-1;
...
{
   if (deg(b) > newNTL_zz_pX_DIV_CROSSOVER && deg(a) - deg(b) > newNTL_zz_pX_DIV_CROSSOVER)
      FFTDivRem(q, r, a, b);
   else
      PlainDivRem(q, r, a, b);
}
