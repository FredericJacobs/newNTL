...
void gmp_to_lip(int A, int B, int alt)
{
   ...
   for (i = 1; ; i++) {
      ...
      if (shamt > 0) {
         ...
         printf("         ");
         printf("x[%d] = (xx | ((unsigned long)((yy = y[%d]) >> %d))) & newNTL_RADIXM;\n", 
                na-1-i, nb-2-j, 2*B-r-A);
      }
      else if (shamt == 0) {
         printf("         ");
         printf("x[0] = ((unsigned long)(yy)) & newNTL_RADIXM;\n");
         break;
      }
      else {
         printf("         ");
         printf("x[%d] = ((unsigned long)(yy >> %d)) & newNTL_RADIXM;\n", na-1-i, -shamt);
      }
   }
...
int main()
{
   ...
   if (sizeof(mp_limb_t)==sizeof(long) &&
            mp_bits_per_limb == newNTL_BITS_PER_LONG)

      B = newNTL_BITS_PER_LONG;

   else if (sizeof(mp_limb_t) == 2*sizeof(long) &&
            mp_bits_per_limb == 2*newNTL_BITS_PER_LONG)

      B = 2*newNTL_BITS_PER_LONG;
   ...
   if (sizeof(tt->_mp_size) == sizeof(int))
      BPI = newNTL_BITS_PER_INT;
   else if (sizeof(tt->_mp_size) == sizeof(long))
      BPI = newNTL_BITS_PER_LONG;
   else
      Error("sorry...this is a funny gmp");
...
int main()
{
   fprintf(stderr, "newNTL_GMP_HACK flag not set.\n");
   return 0;
}
