...
/* Compute a = b * 2^l mod p, where p = 2^n+1. 0<=l<=n and 0<b<p are
   assumed. */
namespaceanon void LeftRotate(ZZ& a, const ZZ& b, long l, const ZZ& p, long n)
{
  if (l == 0) {
    if (&a != &b) {
      a = b;
...
namespaceanon void Rotate(ZZ& a, const ZZ& b, long l, const ZZ& p, long n)
{
  if (IsZero(b)) {
    clear(a);
    return;
...
/* Fast Fourier Transform. a is a vector of length 2^l, 2^l divides 2n,
   ...
   order. */
namespaceanon void fft(vec_ZZ& a, long r, long l, const ZZ& p, long n)
{
  long round;
  long off, i, j, e;
  long halfsize;
...
/* Inverse FFT. r must be the same as in the call to FFT. Result is
   by 2^l too large. */
namespaceanon void ifft(vec_ZZ& a, long r, long l, const ZZ& p, long n)
{
  long round;
  long off, i, j, e;
  long halfsize;
...
newNTL_vector_impl(ZZX,vec_ZZX)

newNTL_eq_vector_impl(ZZX,vec_ZZX)

newNTL_io_vector_impl(ZZX,vec_ZZX)
