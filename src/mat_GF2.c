...
{  
   ...
   long mw = (m + newNTL_BITS_PER_LONG - 1)/newNTL_BITS_PER_LONG;
  
   long i;  
   for (i = 0; i < n; i++) {
      _newntl_ulong *xp = X[i].rep.elts();
      const _newntl_ulong *ap = A[i].rep.elts();
      const _newntl_ulong *bp = B[i].rep.elts();
      long j;
      for (j = 0; j < mw; j++)
         xp[j] = ap[j] ^ bp[j];
   }
...
{  
   ...
   _newntl_ulong a_mask = 1;

   _newntl_ulong *xp = x.rep.elts();

   long lw = (l + newNTL_BITS_PER_LONG - 1)/newNTL_BITS_PER_LONG;
...
void determinant(GF2& d, const mat_GF2& M_in)
{
   ...
   for (k = 0; k < n; k++) {
      long wk = k/newNTL_BITS_PER_LONG;
      long bk = k - wk*newNTL_BITS_PER_LONG;
      _newntl_ulong k_mask = 1UL << bk;
      ...
      if (pos != -1) {
         ...
         _newntl_ulong *y = M[k].rep.elts();

         for (i = k+1; i < n; i++) {
            // M[i] = M[i] + M[k]*M[i,k]

            if (M[i].rep.elts()[wk] & k_mask) {
               _newntl_ulong *x = M[i].rep.elts();
...
long IsUnitVector(const vec_GF2& a, long i)
{
   ...
   const _newntl_ulong *p = a.rep.elts();
...
// ALIAS RESTRICTION: a should not alias any row of x
{
   ...
   long wj = j/newNTL_BITS_PER_LONG;
   long bj = j - wj*newNTL_BITS_PER_LONG;
   _newntl_ulong j_mask = 1UL << bj;
...

{
   ...
   long wn = ((n+1) + newNTL_BITS_PER_LONG - 1)/newNTL_BITS_PER_LONG;
...
void inv(GF2& d, mat_GF2& X, const mat_GF2& A)
{
   ...
   long wn = ((2*n) + newNTL_BITS_PER_LONG - 1)/newNTL_BITS_PER_LONG;
