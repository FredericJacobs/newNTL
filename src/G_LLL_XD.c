...
GivensCache_XD::GivensCache_XD(long m, long n)
{
   ...
   long i;
   buf = newNTL_NEW_OP xdoubleptr[sz]; 
   if (!buf) Error("out of memory");
   for (i = 0; i < sz; i++)
      if (!(buf[i] = newNTL_NEW_OP xdouble[n+1])) Error("out of memory");
           ...
           LLLCheckFct check)
{
   ...
   B1 = newNTL_NEW_OP xdoubleptr[m+1];
   if (!B1) Error("G_LLL_XD: out of memory");

   for (i = 1; i <= m; i++) {
      B1[i] = newNTL_NEW_OP xdouble[n+1];
      if (!B1[i]) Error("G_LLL_XD: out of memory");
   }

   xdouble **mu;
   mu = newNTL_NEW_OP xdoubleptr[m+1];
   if (!mu) Error("G_LLL_XD: out of memory");

   for (i = 1; i <= m; i++) {
      mu[i] = newNTL_NEW_OP xdouble[n+2];
      if (!mu[i]) Error("G_LLL_XD: out of memory");
   }

   xdouble **aux;
   aux = newNTL_NEW_OP xdoubleptr[m+1];
   if (!aux) Error("G_LLL_XD: out of memory");

   for (i = 1; i <= m; i++) {
      aux[i] = newNTL_NEW_OP xdouble[n+1];
         ...
         long beta, long prune, LLLCheckFct check)
{
   ...
   B1 = newNTL_NEW_OP xdoubleptr[m+2];
   ...
   xdouble **mu;
   mu = newNTL_NEW_OP xdoubleptr[m+2];
   ...
   xdouble **aux;
   aux = newNTL_NEW_OP xdoubleptr[m+2];
   ...
   c = newNTL_NEW_OP xdouble[m+2];
   if (!c) Error("G_BKZ_XD: out of memory");

   xdouble cbar;

   xdouble *ctilda;
   ctilda = newNTL_NEW_OP xdouble[m+2];
   if (!ctilda) Error("G_BKZ_XD: out of memory");

   xdouble *vvec;
   vvec = newNTL_NEW_OP xdouble[m+2];
   if (!vvec) Error("G_BKZ_XD: out of memory");

   xdouble *yvec;
   yvec = newNTL_NEW_OP xdouble[m+2];
   if (!yvec) Error("G_BKZ_XD: out of memory");

   xdouble *uvec;
   uvec = newNTL_NEW_OP xdouble[m+2];
   if (!uvec) Error("G_BKZ_XD: out of memory");

   xdouble *utildavec;
   utildavec = newNTL_NEW_OP xdouble[m+2];
