                    ...
                    long half_flag, long short_flag)
{
  ...
  if (fn > 0) /* Mul1/AddMul1 */
    {
      fprintf(f, "   long i;\\\n");
      fprintf(f, "   _newntl_ulong carry = 0, b;\\\n");
    }
  fprintf(f, "   _newntl_ulong hi, lo, t;\\\n");
  fprintf(f, "   _newntl_ulong A[%ld];\\\n", 1L << k);
  fprintf(f, "   A[0] = 0;\\\n");
                        ...
                        long half_flag, long short_flag)
{
  ...
  if (fn > 0) /* Mul1/AddMul1 */
    {
      fprintf(f, "   long i;\\\n");
      fprintf(f, "   _newntl_ulong carry = 0;\\\n");
    }
  fprintf(f, "   _newntl_ulong A[%ld];\\\n", 1L << k);
  fprintf(f, "   A[0] = 0;\\\n");
  ...
  if (fn > 0)
    {
      fprintf(f, "   for (i = 0; i < sb; i++) {\\\n");
      fprintf(f, "      const _newntl_ulong b = bp[i];\\\n");
    }

  for (i = k; i < n - chop; i += k) {
    if (fn > 0) fprintf(f, "   ");
    fprintf(f, "   const _newntl_ulong t%ld = A[(b >> %ld) & %lu]; \\\n", i, i, mask);

  }
  if (fn > 0) fprintf(f, "   ");
  fprintf(f, "   const _newntl_ulong t%ld = A[b >> %ld]; \\\n", n-chop, n-chop);

  if (fn > 0) fprintf(f, "   ");
  fprintf(f, "   const _newntl_ulong lo = A[b & %lu] \\\n", mask);
  ...
  for (i = k; i < n; i += k) {
    if (fn > 0) fprintf(f, "   ");
    if (i == k)
      fprintf(f, "   const _newntl_ulong hi = ");
    else
      fprintf(f, "      ^ ");
...
void print_BB_mul_code(FILE *f, long n)
{
   ...
   fprintf(f, "#define newNTL_BB_MUL1_BITS (4)\n\n");
}
...
void print_BB_sqr_code(FILE *f, long n)
{
   ...
   fprintf(f, "\n\n");
   fprintf(f, "#define newNTL_BB_SQR_CODE \\\n");
   ...
   for (i = 8; i < n; i += 8) {
      if (2*(i+8) <= n) {
         ...
         fprintf(f, "_newntl_ulong t=sqrtab[(a>>%ld)&255];\\\n", i);
...
void print_BB_rev_code(FILE *f, long n)
{
   ...
   fprintf(f, "\n\n");
   fprintf(f, "#define newNTL_BB_REV_CODE ");
...
int main()
{
   ...
   fprintf(stderr, "This is newNTL version %s\n\n", newNTL_VERSION);
    ...
    * properties hold.  newNTL relies implicitly and crucially on 
    * these properties.
    *
    ...
    * predict the general behavior.  One should set the newNTL_CLEAN_INT
    ...
    * you will have to set the newNTL_EXT_DOUBLE in "mach_desc.h"
    * by hand.
    * 
   ...
   /*
    * Set nbits --- the default radix size for newNTL's "built in"
   ...
   /* 
    * We next test if the newNTL_SINGLE_MUL option is valid.  This test is
    ...
    * bad behavior, especially if the newNTL_SINGLE_MUL option is never used.
   ...
   if (((dp1 > dp) || dr) && !AutoFix) {
      ...
      fprintf(stderr, "to compile newNTL with the newNTL_X86_FIX flag to get true IEEE floating point.\n");
   ...
   fprintf(f, "#ifndef newNTL_mach_desc__H\n");
   fprintf(f, "#define newNTL_mach_desc__H\n\n\n");
   fprintf(f, "#define newNTL_BITS_PER_LONG (%ld)\n", bpl);
   fprintf(f, "#define newNTL_MAX_LONG (%ldL)\n", ((long) ((1UL<<(bpl-1))-1UL)));
   fprintf(f, "#define newNTL_MAX_INT (%ld)\n", ((long) ((1UL<<(bpi-1))-1UL)));
   fprintf(f, "#define newNTL_BITS_PER_INT (%ld)\n", bpi);
   fprintf(f, "#define newNTL_BITS_PER_SIZE_T (%ld)\n", bpt);
   fprintf(f, "#define newNTL_ARITH_RIGHT_SHIFT (%ld)\n", rs_arith);
   fprintf(f, "#define newNTL_NBITS_MAX (%ld)\n", nbits);
   fprintf(f, "#define newNTL_DOUBLE_PRECISION (%ld)\n", dp);
   fprintf(f, "#define newNTL_FDOUBLE_PRECISION ");
   print2k(f, dp-1, bpl);
   fprintf(f, "\n");
   fprintf(f, "#define newNTL_QUAD_FLOAT_SPLIT (");
   print2k(f, dp - (dp/2), bpl);
   fprintf(f, "+1.0)\n");
   fprintf(f, "#define newNTL_EXT_DOUBLE (%d)\n", ((dp1 > dp) || dr));
   fprintf(f, "#define newNTL_SINGLE_MUL_OK (%d)\n", single_mul_ok != 0);
   fprintf(f, "#define newNTL_DOUBLES_LOW_HIGH (%d)\n\n\n", single_mul_ok < 0);
   print_BB_mul_code(f, bpl);
   print_BB_sqr_code(f, bpl);
   print_BB_rev_code(f, bpl);

   fprintf(f, "#define newNTL_MIN_LONG (-newNTL_MAX_LONG - 1L)\n");
   fprintf(f, "#define newNTL_MIN_INT  (-newNTL_MAX_INT - 1)\n");
