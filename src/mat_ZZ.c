...
newNTL_matrix_impl(ZZ,vec_ZZ,vec_vec_ZZ,mat_ZZ)
newNTL_io_matrix_impl(ZZ,vec_ZZ,vec_vec_ZZ,mat_ZZ)
newNTL_eq_matrix_impl(ZZ,vec_ZZ,vec_vec_ZZ,mat_ZZ)
...
void solve1(ZZ& d_out, vec_ZZ& x_out, const mat_ZZ& A, const vec_ZZ& b)
{
   ...
   if (max_A_len + newNTL_SP_NBITS + NumBits(n) <= newNTL_DOUBLE_PRECISION-1)
      use_double_mul1 = 1;

   if (!use_double_mul1 && max_A_len+newNTL_SP_NBITS+2 <= newNTL_DOUBLE_PRECISION-1) {
      use_double_mul2 = 1;
      double_limit = (1L << (newNTL_DOUBLE_PRECISION-1-max_A_len-newNTL_SP_NBITS));
   }
   ...
   if (max_A_len + newNTL_SP_NBITS + NumBits(n) <= newNTL_BITS_PER_LONG-1)
      use_long_mul1 = 1;

   if (!use_long_mul1 && max_A_len+newNTL_SP_NBITS+2 <= newNTL_BITS_PER_LONG-1) {
      use_long_mul2 = 1;
      long_limit = (1L << (newNTL_BITS_PER_LONG-1-max_A_len-newNTL_SP_NBITS));
   }
   ...
   if (use_double_mul1 || use_double_mul2) {
      double_h = newNTL_NEW_OP double[n];
      double_A = newNTL_NEW_OP double_ptr[n];
      if (!double_h || !double_A) Error("solve1: out of mem");

      for (i = 0; i < n; i++) {
         double_A[i] = newNTL_NEW_OP double[n];
   ...
   if (use_long_mul1 || use_long_mul2) {
      long_h = newNTL_NEW_OP long[n];
      long_A = newNTL_NEW_OP long_ptr[n];
      if (!long_h || !long_A) Error("solve1: out of mem");

      for (i = 0; i < n; i++) {
         long_A[i] = newNTL_NEW_OP long[n];
