...
namespaceanon void RowTransform(vec_ZZ& A, vec_ZZ& B, const ZZ& MU1)
// x = x - y*MU
{
   staticthread ZZ T, MU;
   long k;
...
#define TR_BND (newNTL_FDOUBLE_PRECISION/2.0)
// Just to be safe!!

namespaceanon double max_abs(double *v, long n)
{
   long i;
   double res, t;
...
namespaceanon void RowTransformStart(double *a, long *in_a, long& in_float, long n)
{
   long i;
   long inf = 1;
...
namespaceanon void RowTransformFinish(vec_ZZ& A, double *a, long *in_a)
{
   long n = A.length();
   long i;
...
namespaceanon void RowTransform2(vec_ZZ& A, vec_ZZ& B, const ZZ& MU1)
// x = x + y*MU
...
GivensCache_FP::GivensCache_FP(long m, long n)
{
   ...
   long i;
   buf = newNTL_NEW_OP doubleptr[sz]; 
   if (!buf) Error("out of memory");
   for (i = 0; i < sz; i++)
      if (!(buf[i] = newNTL_NEW_OP double[n+1])) Error("out of memory");

   bl = newNTL_NEW_OP long[sz];
   if (!bl) Error("out of memory");
   for (i = 0; i < sz; i++) bl[0] = 0;

   bv = newNTL_NEW_OP long[sz];
...
staticthread double red_fudge = 0;
staticthread long log_red = 0;

staticthread long verbose = 0;

staticthread unsigned long NumSwaps = 0;
staticthread double StartTime = 0;
staticthread double LastTime = 0;



namespaceanon void G_LLLStatus(long max_k, double t, long m, const mat_ZZ& B)
{
...
namespaceanon void init_red_fudge()
{
   long i;

   log_red = long(0.50*newNTL_DOUBLE_PRECISION);
   red_fudge = 1;
...
namespaceanon void inc_red_fudge()
{
...
namespaceanon void print_mus(double **mu, long k)
{
   long i;
           ...
           long m, long init_k, long &quit, GivensCache_FP& cache)
{
   ...
   double *max_b;
   max_b = newNTL_NEW_OP double [m+1];
           ...
           LLLCheckFct check)
{
   ...
   B1 = newNTL_NEW_OP doubleptr[m+1];
   if (!B1) Error("G_LLL_FP: out of memory");

   for (i = 1; i <= m; i++) {
      B1[i] = newNTL_NEW_OP double[n+1];
      if (!B1[i]) Error("G_LLL_FP: out of memory");
   }

   double **mu;
   mu = newNTL_NEW_OP doubleptr[m+1];
   if (!mu) Error("G_LLL_FP: out of memory");

   for (i = 1; i <= m; i++) {
      mu[i] = newNTL_NEW_OP double[n+2];
      if (!mu[i]) Error("G_LLL_FP: out of memory");
   }

   double **aux;
   aux = newNTL_NEW_OP doubleptr[m+1];
   if (!aux) Error("G_LLL_FP: out of memory");

   for (i = 1; i <= m; i++) {
      aux[i] = newNTL_NEW_OP double[n+1];
...
namespaceanon inline vec_double& G_BKZConstant() {
  staticthread vec_double m_G_BKZConstant;
  return m_G_BKZConstant;
}
...
namespaceanon inline vec_double& G_BKZThresh() {
  staticthread vec_double m_G_BKZThresh;
  return m_G_BKZThresh;
}
         ...
         long beta, long prune, LLLCheckFct check)
{
   ...
   B1 = newNTL_NEW_OP doubleptr[m+2];
   ...
   double **mu;
   mu = newNTL_NEW_OP doubleptr[m+2];
   if (!mu) Error("G_LLL_FP: out of memory");
   ...
   double **aux;
   aux = newNTL_NEW_OP doubleptr[m+2];
   if (!aux) Error("G_LLL_FP: out of memory");
