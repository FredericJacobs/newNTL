...
#ifndef newNTL_vec_RR__H
#define newNTL_vec_RR__H
...
newNTL_vector_decl(RR,vec_RR)

newNTL_eq_vector_decl(RR,vec_RR)

newNTL_io_vector_decl(RR,vec_RR)
...
inline vec_RR VectorCopy(const vec_RR& a, long n)
   { vec_RR x; VectorCopy(x, a, n); newNTL_OPT_RETURN(vec_RR, x); }
...
inline vec_RR operator*(const vec_RR& a, const RR& b)
   { vec_RR x; mul(x, a, b); newNTL_OPT_RETURN(vec_RR, x); }
