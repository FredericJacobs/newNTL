...
#ifndef newNTL_mat_ZZ__H
#define newNTL_mat_ZZ__H
...
inline mat_ZZ transpose(const mat_ZZ& A)
   { mat_ZZ x; transpose(x, A); newNTL_OPT_RETURN(mat_ZZ, x); }

void conv(mat_zz_p& x, const mat_ZZ& a);
template<> inline mat_zz_p to<mat_zz_p>(const mat_ZZ& a)
   { mat_zz_p x; conv(x, a); newNTL_OPT_RETURN(mat_zz_p, x); }
...
inline mat_ZZ operator*(const mat_ZZ& a, const ZZ& b)
   { mat_ZZ x; mul(x, a, b); newNTL_OPT_RETURN(mat_ZZ, x); }
