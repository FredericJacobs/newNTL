...
#ifndef newNTL_ZZ_pEXFactoring__H
#define newNTL_ZZ_pEXFactoring__H

#include <newNTL/pair_ZZ_pEX_long.h>
...
inline ZZ_pEX BuildIrred_ZZ_pEX(long n)
   { ZZ_pEX x; BuildIrred(x, n); newNTL_OPT_RETURN(ZZ_pEX, x); }
...
inline ZZ_pEX BuildRandomIrred(const ZZ_pEX& g)
    { ZZ_pEX x; BuildRandomIrred(x, g); newNTL_OPT_RETURN(ZZ_pEX, x); }
...
// Space allocation can be controlled via ComposeBound (see <newNTL/ZZ_pEX.h>)
