...
#ifndef newNTL_vec_GF2__H
#define newNTL_vec_GF2__H

#include <newNTL/WordVector.h>
#include <newNTL/GF2.h>
...
//the following are "really" public
   ...
   long allocated() const { return rep.MaxLength() * newNTL_BITS_PER_LONG; }
...
newNTL_SNS ostream& operator<<(newNTL_SNS ostream& s, const vec_GF2& a);
newNTL_SNS istream& operator>>(newNTL_SNS istream& s, vec_GF2& a);
...
inline vec_GF2 shift(const vec_GF2& a, long n)
   { vec_GF2 x; shift(x, a, n); newNTL_OPT_RETURN(vec_GF2, x); }

void reverse(vec_GF2& x, const vec_GF2& a);

inline vec_GF2 reverse(const vec_GF2& a)
   { vec_GF2 x; reverse(x, a); newNTL_OPT_RETURN(vec_GF2, x); }

void random(vec_GF2& x, long n);
inline vec_GF2 random_vec_GF2(long n)
   { vec_GF2 x; random(x, n); newNTL_OPT_RETURN(vec_GF2, x); }
...
inline vec_GF2 operator*(const vec_GF2& a, GF2 b)
   { vec_GF2 x; mul(x, a, b); newNTL_OPT_RETURN(vec_GF2, x); }
