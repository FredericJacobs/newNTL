...
zz_pContext() { ptr = 0; }
zz_pContext(long p, long maxroot=newNTL_FFTMaxRoot);
...
#define newNTL_zz_pRegister(x) zz_p x
...
static void init(long NewP, long maxroot=newNTL_FFTMaxRoot);
...
newNTL_SNS ostream& operator<<(newNTL_SNS ostream& s, zz_p a);
