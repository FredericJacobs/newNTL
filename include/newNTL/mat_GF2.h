...
#ifndef newNTL_mat_GF2__H
#define newNTL_mat_GF2__H
...
inline mat_GF2 to_mat_GF2(const vec_vec_GF2& a)
   { mat_GF2 x; conv(x, a); newNTL_OPT_RETURN(mat_GF2, x); }
...
newNTL_SNS istream& operator>>(newNTL_SNS istream&, mat_GF2&); 
newNTL_SNS ostream& operator<<(newNTL_SNS ostream&, const mat_GF2&);  
...
inline mat_GF2 ident_mat_GF2(long n)
   { mat_GF2 X; ident(X, n); newNTL_OPT_RETURN(mat_GF2, X); }
...
inline mat_GF2 diag(long n, GF2 d)
   { mat_GF2 X; diag(X, n, d); newNTL_OPT_RETURN(mat_GF2, X); }
...
inline mat_GF2 transpose(const mat_GF2 & a)
   { mat_GF2 x; transpose(x, a); newNTL_OPT_RETURN(mat_GF2, x); }
...
inline mat_GF2 operator*(const mat_GF2& a, GF2 b)
   { mat_GF2 x; mul(x, a, b); newNTL_OPT_RETURN(mat_GF2, x); }
